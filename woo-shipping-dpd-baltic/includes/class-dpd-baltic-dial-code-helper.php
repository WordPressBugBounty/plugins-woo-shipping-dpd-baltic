 ...
 * @author     DPD
 */
class Dpd_Baltic_Dial_Code_Helper {
	 ...
	 *
	 * @param string $country_id ISO-3166 country code.
	 *
	 * @return string
	 */
	public function get_country_code( $country_id ) {
		if ( isset( $this->country_codes[ $country_id ] ) ) {
			return $this->country_codes[ $country_id ];
		}
	 ...
	 * @return array
	 */
	public function separate_phone_number_from_country_code( $phonenumber, $country_id ) {
		$result            = array(
			'dial_code'    => '',
			'phone_number' => '',
		);
		$default_dial_code = $this->get_country_code( $country_id );
		...
		$contains_country_code = strpos( $phonenumber, $default_dial_code ) === 0 || strpos( $phonenumber, '+' ) === 0 || strpos( $phonenumber, '00' ) === 0 || strlen( $phonenumber ) > 10;

		// when country code is not supplied, then it can
		// start with single zero
		// start with any number.
		if ( ! $contains_country_code ) {
			$result['dial_code'] = '+' . $default_dial_code;
			if ( strpos( $phonenumber, '0' ) === 0 ) {
				$result['phone_number'] = substr( $phonenumber, 1 );
			} elseif ( '8' === $phonenumber[0] ) {
				$result['phone_number'] = substr( $phonenumber, 1 );
			} else {
			...
			$dial_code              = $this->get_country_code_from_phonenumber( $phonenumber, $default_dial_code );
			$result['dial_code']    = '+' . $dial_code;
			$result['phone_number'] = substr( $phonenumber, strlen( $dial_code ) );
		}
	 ...
	 * @param string $default_code dial code to be used if no country code is found.
	 *
	 * @return string resulting dial code without + sign
	 */
	public function get_country_code_from_phonenumber( $phonenumber, $default_code ) {
		$matching_country_code = '';

		foreach ( $this->get_country_codes() as $country_iso => $dial_code ) {
			if ( strpos( $phonenumber, $dial_code ) === 0 ) {
				if ( strlen( $dial_code ) > $matching_country_code ) {
					$matching_country_code = $dial_code;
				}
			}
		}
